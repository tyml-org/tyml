...
use crate::{server::rust_axum::{crate_gen::generate_cargo_toml, function_gen::generate_functions, lib_gen::generate_lib}, GeneratorSettings};
...
pub fn generate_rust_axum_server(setting: &GeneratorSettings, tyml: &Tyml) -> Result<(), Box<dyn std::error::Error>> {
    ...
    generate_functions(setting, tyml)?;
